...
class myVMLSTMCell_NEO_final_group2(nn.Module):
    

    def __init__(self, input_size, hidden_size, wRank=None, uRanks=None, g=2, recurrent_init=None,hidden_init=None):
        super(myVMLSTMCell_NEO_final_group2, self).__init__()
        print("last updated 0914_finalgroups")
    ...
    def forward(self, x, hiddenStates,device):
        ...
        for i in range(self.g):
            ...
            if i==0: #first group
                #g_h=h_op.contiguous().view(batch_size,self.hidden_size*4) 
                f_h,i_h,n_h,o_h=h_op.chunk(4,dim=2)
                #print(f"shape of f_h [batch,g,h/g - 81,2,90]->{f_h.shape}")
                #print(f"shape of i_h [batch,g,h/g - 81,2,90]->{i_h.shape}")
                #print(f"shape of n_h [batch,g,h/g - 81,2,90]->{n_h.shape}")
                #print(f"shape of o_h [batch,g,h/g - 81,2,90]->{o_h.shape}")
            else:
                tf_h,ti_h,tn_h,to_h=h_op.chunk(4,dim=2)
                #print(f"shape of f_h [batch,g,h/g - 81,2,90]->{tf_h.shape}")
                #print(f"shape of i_h [batch,g,h/g - 81,2,90]->{ti_h.shape}")
                #print(f"shape of n_h [batch,g,h/g - 81,2,90]->{tn_h.shape}")
                #print(f"shape of o_h [batch,g,h/g - 81,2,90]->{to_h.shape}")
                f_h=f_h+tf_h; i_h=i_h+ti_h; n_h=n_h+tn_h; o_h=o_h+to_h
